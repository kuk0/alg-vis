...
public class Node implements VisualElement {
	 ...
	 */
	public static final int INF = 99999, NOKEY = -99998, NULL = 100000;
	/**
	...
	public Node(DataStructure D, int key, float x, float y) {
		this.D = D;
		this.key = key;
	...
	public Node(Node v) {
		this(v.D, v.key, v.pos.x, v.pos.y);
	}
	...
	public void goTo(int tox, int toy) {
		if (D.scenario.traverser.isInterrupted()) {
			pos.moveTo(new FixedPos(tox, toy));
		} else if (pos.tx != tox || pos.ty != toy) {
			if (D.scenario.isAddingEnabled()) {
	...
	public void goAbove(Node v) {
		pos.moveTo(new RelPos(v.pos, 0, - DataStructure.minsepy));
	}
	...
	public Rectangle2D boundingBox() {
		return new Rectangle2D.Float(pos.x - 10, pos.y - 10, pos.x + 10,
				pos.y + 10);
	}
