...
abstract class DynamicArrayAlg extends Algorithm {
    ...
    public void createNewArray(int capacity) {
        for (DynamicArrayCoin coin : D.coinsForArray) {
            coin.setColor(NodeColor.RED);
        }
        addStep(D.coinsForArray.get(D.coinsForArray.size() / 2), REL.TOP,
            "dynamicarray-use-coin");
        pause();
        ...
        addStep(D.newarray.get(0), REL.TOPRIGHT, "dynamicarray-new");

        if (capacity >= 4) {
            D.newdelimiter4 = new DynamicArrayDelimiter(D,
                D.newarray.get(capacity / 4), Color.GREEN);
            D.newdelimiter2 = new DynamicArrayDelimiter(D,
                D.newarray.get(capacity / 2), Color.RED);
        }
        pause();
        if (D.size > 0) {
            addStep(D.newarray.get(0), REL.TOPRIGHT, "dynamicarray-copy");
        }
