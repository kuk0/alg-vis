...
public class FenwickPrefixSum extends Algorithm {
	...
	/**
	 * Returns decimal idx/newidx and binary idx/newidx with highlighted changed
	 * bit, formatted to be as long as idxmax
	 * 
	...
	private static String[] getPrettyIdx(int idx, int idxmax) {
		...
		sidx = sidx.substring(0, length - i) + "<u>1</u>"
				+ sidx.substring(length - i + 1);
		snewidx = snewidx.substring(0, length - i) + "<u>0</u>"
				+ snewidx.substring(length - i + 1);
