...
public class Edge extends VisualElement {
    ...
    public Edge(Node u, Node m, Node v) {
        ...
        if ((u.x < m.x) != (m.x < v.x)) {
            ...
            double vecX = Math.abs(u.x - v.x), vecY = Math.abs(u.y - v.y),
                d = Math.sqrt(vecX * vecX + vecY * vecY), mx = m.x, my = m.y;
